        ...
        float b)
{
    ...
    film->set(cx, film->get_height() - 1 - cy, std::move(item));
}
...
std::unique_ptr<PfmItem> IntensityFilm::get_image_coord(int x, int y) {
    return film->get(x, y)->clone();
}

// Get a pixel
// Uses camera coordinates: Y is from bottom to top
std::unique_ptr<PfmItem> IntensityFilm::get_camera_coord(int x, int y) {
    return film->get(x, film->get_height() - 1 - y)->clone();
}

// Get a pixel
// Uses camera coordinates: Y is from bottom to top
// Multiplies value by jacobian factor
std::unique_ptr<PfmItem> IntensityFilm::get_camera_coord_jacobian(int x, int y) {
    std::unique_ptr<PfmItem> pix = get_camera_coord(x, y)->clone();
