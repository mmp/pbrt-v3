...
Float AdaptiveSimpson(const std::function<Float (Float)> &f, Float x0, Float x1, Float eps = 1e-6f, int depth = 6) {
    ...
    std::function<Float (Float, Float, Float, Float, Float, Float, Float, Float, int)> integrate =
        ...
        Float h = c-a,
              I0 = (Float) (1.0/12.0) * h * (fa + 4*fd + fb),
...
void FrequencyTable(const BSDF *bsdf, const Vector3f &wo, RNG &rng, int sampleCount, int thetaRes, int phiRes, Float *target) {
    memset(target, 0, thetaRes*phiRes*sizeof(Float));

    Float factorTheta = thetaRes / Pi,
          factorPhi   = phiRes / (2 * Pi);
    ...
    for (int i=0; i<sampleCount; ++i) {
        ...
        if (f == Spectrum() || (flags & BSDF_SPECULAR))
            continue;

        Vector3f wiL = bsdf->WorldToLocal(wi);

        Point2f coords(
            std::acos(wiL.z) * factorTheta,
        ...
        int thetaBin = std::min(std::max(0,
            (int) std::floor(coords.x)), thetaRes-1);
        int phiBin = std::min(std::max(0,
            (int) std::floor(coords.y)), phiRes-1);
...
void IntegrateFrequencyTable(const BSDF *bsdf, const Vector3f &wo, int sampleCount, int thetaRes, int phiRes, Float *target) {
    memset(target, 0, thetaRes*phiRes*sizeof(Float));

    Float factorTheta = Pi / thetaRes,
          factorPhi   = (2 * Pi) / phiRes;

    for (int i=0; i<thetaRes; ++i) {
        for (int j=0; j<phiRes; ++j) {
            *target++ = sampleCount * AdaptiveSimpson2D(
                [&](Float theta, Float phi) -> Float {
                    ...
                    Vector3f wiL(
                        ...
                        sinTheta*sinPhi,
                        cosTheta);
                    return bsdf->Pdf(wo, bsdf->LocalToWorld(wiL), BSDF_ALL) * sinTheta;
                }, i*factorTheta, j*factorPhi, (i+1) * factorTheta, (j+1) * factorPhi);
        }
    }
}

/// Write the frequency tables to disk in a format that is nicely plottable by Octave and MATLAB
void DumpTables(const Float *frequencies, const Float *expFrequencies, int thetaRes, int phiRes, const char *filename) {
...
bool Chi2Test(const Float *frequencies, const Float *expFrequencies, int thetaRes, int phiRes,
    ...
    for (const Cell &c : cells) {
        if (expFrequencies[c.index] == 0) {
            if (frequencies[c.index] > sampleCount * 1e-5f) {
                ...
                std::cout << "  Encountered " << frequencies[c.index] << " samples in a c "
    ...
    if (pooledExpFrequencies > 0 || pooledFrequencies > 0) {
        std::cout << "  Pooled " << pooledCells << " to ensure sufficiently high expected "
    ...
    if (dof <= 0) {
        std::cout << "The number of degrees of freedom (" << dof << ") is too low!" << std::endl;
        return false;
    }

    std::cout << "  Chi-square statistic = " << chsq << " (d.o.f. = " << dof << ")" << std::endl;
    ...
    if (pval < alpha || !std::isfinite(pval)) {
        std::cout << "  ***** Rejected ***** the null hypothesis (p-value = " << pval << ", "
            "significance level = " << alpha << ")" << std::endl;
        return false;
    } else {
        std::cout << "  Accepted the null hypothesis (p-value = " << pval << ", "
            "significance level = " << alpha << ")" << std::endl;
        return true;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    int passed = 0, total = numModels * CHI2_RUNS, index = 0;
    ...
    for (int model = 0; model < numModels; model++) {
        ...
        for (int k = 0; k < CHI2_RUNS; ++k) {
            std::cout << "Testing " << BSDFFuncDescripArray[model] << " (test "
                      << k+1 << "/" << CHI2_RUNS << ") .." << std::endl;
            ...
            std::cout << "  Accumulating " << sampleCount << " samples into a " << thetaRes
                 << "x" << phiRes << " frequency table .." << std::endl;
            FrequencyTable(bsdf, wo, rng, sampleCount, thetaRes, phiRes, frequencies);

            std::cout << "  Integrating expected frequencies .." << std::endl;
            IntegrateFrequencyTable(bsdf, wo, sampleCount, thetaRes, phiRes, expFrequencies);

            char filename[32];
            snprintf(filename, sizeof(filename), "/tmp/chi2test_%03i.m", ++index);
            std::cout << "  Writing snapshot to \"" << filename << "\" .." << std::endl;
            DumpTables(frequencies, expFrequencies, thetaRes, phiRes, filename);

            passed += Chi2Test(frequencies, expFrequencies, thetaRes, phiRes,
                sampleCount, CHI2_MINFREQ, CHI2_SLEVEL, total) ? 1 : 0;

            std::cout << std::endl;
        }
    }
    std::cout << "Done (passed " << passed << "/" << total << " tests)." << std::endl;

    delete[] frequencies;
    delete[] expFrequencies;

    pbrtCleanup();
    return (passed != numModels * CHI2_RUNS);
}
